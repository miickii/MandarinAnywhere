...
const WordForm = ({ word, onClose, add=true }) => {
    ...
    return (
        <form className="h-full w-full p-5 bg-white border border-gray-400" onSubmit={addToDB}>
            <IoIosClose size={40} className="cursor-pointer absolute top-0 right-0 mt-2 mr-3 text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out" onClick={() => onClose()} />

            <h1 className="text-gray-800 text-xl font-bold mb-4">{add ? "Add Word" : "Edit Word"}</h1>

            <div className="flex w-full justify-between">
                <div className="w-[45%]">
                    <label className="text-gray-800 text-sm font-bold">Characters</label>
                    <input id="chinese" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-sky-blue font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" value={chinese} onChange={e => setChinese(e.target.value)} />
                </div>
                <div className="w-[45%]">
                    <label className="text-gray-800 text-sm font-bold">Pinyin</label>
                    <input id="pinyin" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-sky-blue font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" value={pinyin} onChange={e => setPinyin(e.target.value)} />
                </div>
            </div>
            
           
            <label className="text-gray-800 text-sm font-bold">English</label>
            <input id="english" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-sky-blue font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" value={english} onChange={e => setEnglish(e.target.value)} />
            ...
            <h1 className="text-gray-800 text-md font-bold">Example sentence</h1>
            <label className="text-gray-800 text-sm font-bold">Characters</label>
            ...
            <div className="flex items-center justify-center w-full pt-2">
                <button type="submit" className="focus:outline-none focus:ring-1 focus:ring-offset-2 focus:ring-sky-blue transition duration-150 ease-in-out hover:bg-sky-blue-dark bg-interactives rounded text-white px-8 py-2 mr-4 text-sm">
                    {!loading && (
                        <h1>{add ? "Add" : "Edit"}</h1>
                    )}
                    {loading && (
                ...
                <button className="focus:outline-none focus:ring-1 focus:ring-offset-2 focus:ring-gray-400 bg-gray-100 transition duration-150 text-gray-600 ease-in-out hover:border-gray-400 hover:bg-gray-300 border rounded px-8 py-2 text-sm" onClick={() => onClose()}>Cancel</button>
            </div>
        </form>
    )
}
